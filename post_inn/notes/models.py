...
class Tag(Model):
    ...
    @staticmethod
    def get_tag(user_pk):
        return Tag.objects.filter(is_active=True, author=user_pk)
    ...
    def get_author_tag(request, tag_pk):
        ...
        """
        return Tag.objects.values_list('author').filter(author=request.user, is_active=True, id=tag_pk).first()
...
class Note(Model):
    ...
    tags = ManyToManyField(Tag, verbose_name='Тэг', blank=True)
    created = DateTimeField(verbose_name='Дата и время создания', default=datetime.now)
    update = DateTimeField(verbose_name='Дата обновления', auto_now=True)
